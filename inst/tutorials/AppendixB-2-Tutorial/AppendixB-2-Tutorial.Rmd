...
grade_this({
  # custom checking code appears here
  if (identical(.result, .solution)) {
    pass(paste(random_praise(), " Notice that the covariances are slightly different. This is because to calculate the covariance for the sample, we should divide by (n - 1) instead of by n (just like we do with the sample variance). Why?"))
  }
  fail(random_encouragement())
})
```
