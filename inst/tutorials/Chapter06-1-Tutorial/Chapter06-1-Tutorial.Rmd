...
grade_this({if (!inherits(.result, c("character"))) {
  ...
  if (.result[8] != "cigs                            -0.463***  -0.029***           "){ 
    fail("The 'cigs' coefficients don't look quite right.") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("character"))) {
    fail("Your class of your answer should be a list of character strings.")}
  if (.result[8] != "scale(cigs)                      -0.136***      -0.136***                "){ 
    fail("The 'cigs' coefficients don't look quite right.") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("numeric"))) {
    fail("Your class of your answer should be a numeric vector.")}
  if (round(.result,4) != c(-0.1360, 0.0854)){ 
    fail("The coefficients don't look quite right.") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("numeric"))) {
    fail("Your class of your answer should be a numeric vector.")}
  if (round(.result,4) != c(-0.9672, 2.6927)){ 
    fail("The coefficients don't look quite right.") }
  pass()
})
```
...
Estimate the full and partial elasticities for the `bwght.lm1` estimates by summarizing the results of `marginaleffects(model)` using the `marginaleffects` package. 
...
```{r eAME-check}
grade_this({if (!inherits(.result, c("marginaleffects.summary"))) {
    fail("Your class of your answer should be a summary of a marginaleffects object.")}
  if (round(.result[,4],4) != c(-0.9672, 2.6927)){ 
    fail("The coefficients don't look quite right.") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("character"))) {
    fail("Your class of your answer should be a list of character strings.")}
  if (.result[8] != "cigs                            -0.004***  -0.467***  -0.004*** "){
    fail("The 'cigs' coefficients don't look quite right.") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("summary.lm"))) {
  ...
  if (round(.result[2,1]$coefficients,4) != -0.0285){
    fail("The 'cigs' coefficient doesn't look quite right.") }
  pass()
})
```
