...
# "function(x)" works just fine within "integrate()" to define x[f(x)] and x^2[f(x)]. 
...
```{r continuousVx-hint}
# Use the "$" operator to extract the value of the integral used to calculate E(X)
# Use "function" inside "integrate" to get the variance. 
```
