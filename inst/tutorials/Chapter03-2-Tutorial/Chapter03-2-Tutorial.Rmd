...
Return a matrix with the first row equal to the estimated coefficients and the second row equal to their standard errors. 
...
grade_this({if (!inherits(.result, c("matrix"))) {
  ...
  if (round(.result[2,1], 4) != 0.6443) { 
    fail("The coefficient on education should be about 0.64") }
  pass()
})
```
