...
grade_this({if (!inherits(.result, c("character"))) {
  ...
  if (.result[8] != "athlete                                           0.211***                  0.203***                   0.123**         ") { 
    fail("The coefficients for athletes don't look quite right.") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("summary.lm"))) {
  ...
  if (round(.result$coefficients[3],4) != 0.7193) { 
    fail("The coefficient on the price of regular apples should be about 0.72.") }
  pass()
})
```
