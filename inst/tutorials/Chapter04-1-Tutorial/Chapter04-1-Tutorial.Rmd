...
grade_this({if (!inherits(.result, c("summary.lm"))) {
  ...
  if (round(.result$coef[2],4) != 0.5990) { 
    fail("The coefficients on education should be about 0.599") }
  pass()
})
```
...
grade_this({if (!inherits(.result, c("summary.lm"))) {
    fail("Your class of your answer should be summary of an `lm` object.")}
  if (round(.result$coef[2],4) != 262.1805) { 
    fail("The coefficients on education should be about 262.18") }
  pass()
})
```
...
2. Calculate 99-percent confidence intervals for the coefficients.
...
grade_this({if (!inherits(.result, c("matrix"))) {
  ...
  if (round(.result$coef[2,1],4) != 0.3891) { 
    fail("The coefficients on education should be about 0.389") }
  pass()
})
```
