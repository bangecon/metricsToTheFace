...
# Integrate the function (you can actually even do all of this in one line).
```
...
# You may or may not want to define the functions, x*f(x) and x^2*f(x), ahead of 
# integrating.  
...
```{r continuousVx-hint}
# Since we didn't assign E(x) to a named obect, you'll need to define it (and name it). 
# Integrate (x - mu)^2 * f(x) over the sample space.
```
